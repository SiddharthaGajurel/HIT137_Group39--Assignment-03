...
class ModelInfo:
    """Encapsulates model information - private attribute for info."""
    def __init__(self):
        self._info = {
            'sentiment': 'DistilBERT for sentiment analysis: Classifies text as positive/negative. Small, efficient model from NLP category.',
            'image': 'CLIP for image classification: Describes or classifies images. From computer vision category, lightweight.',
            'text_to_image': 'Stable Diffusion v1-5: Generates images from text. From text-to-image category, requires diffusers library.'
        }
...
class BaseModel:
    """Base class for models - for polymorphism and overriding."""
    def __init__(self):
        self.pipe = None
...
class SentimentModel(BaseModel):
    """Inherits from BaseModel, overrides load and predict."""
    def load(self):
        try:
...
class ImageModel(BaseModel):
    """Another model, overrides methods."""
    def load(self):
        try:
...
class TextToImageModel(BaseModel):
    """Third model, overrides. Uses diffusers for Stable Diffusion."""
    def load(self):
        try:
            self.pipe = StableDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5")
            self.pipe = self.pipe.to("cpu")  # Use CPU; change to "cuda" if you have a GPU
        except Exception as e:
    ...
    def predict(self, text):
        ...
        try:
            if self.pipe:
                # Generate image (simplified; saves to file for now)
...
class Logging:
    """Mixin for logging - multiple inheritance example."""
    def log(self, message):
        print(f"Log: {message}")

class AIModel(Logging, BaseModel):  # Multiple inheritance
    """Main model class using multiple inheritance."""
...
class OOPExplainer:
    """Explains OOP concepts - encapsulation via private methods."""
    def _explain_multiple_inheritance(self):
        return "Multiple inheritance: AIModel inherits from Logging (for logging) and BaseModel (for model behavior). Combines behaviors cleanly."

    def _explain_method_overriding(self):
        return "Method overriding: Subclasses (e.g., SentimentModel) override load() and predict() from BaseModel for specific model logic."

    def _explain_polymorphism(self):
        return "Polymorphism: Each model subclass uses the same predict() interface but processes inputs differently (text, image, etc.)."

    def _explain_encapsulation(self):
        return "Encapsulation: ModelInfo hides _info dict; accessed via get_info(). Private methods in OOPExplainer protect explanation logic."

    def _explain_multiple_decorators(self):
        return "Multiple decorators: button_click_decorator logs button clicks. Could add more like @log_time for timing."
...
class AIApp:
    ...
    def on_type_change(self, event=None):
        self.input_data = None  # Reset input when type changes
